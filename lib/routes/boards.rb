module Iceberg
  class App < Sinatra::Base
    ...
    get :boards do
      @boards = model_for(:Board).filtered(current_user).ordered(:parent_id => nil)
      haml :'boards/index'
    end
    ...
    get :new_board do |id|
      @parent = model_for(:Board).filtered(current_user).first(:id => id.to_i)
    ...
    get :edit_board do |id|
      @board = model_for(:Board).filtered(current_user).first(:id => id.to_i)
    ...
    get :board_atom do
      @board = model_for(:Board).by_ancestory(current_user, split_splat)
