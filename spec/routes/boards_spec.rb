...
describe "Boards Routes" do
  ...
  describe "viewing the ATOM feed" do
    before(:each) do
      ...
      user2 = Iceberg::App::User.new(:id => 2, :name => "Brett Gurewitz", :ip_address => "127.0.0.1")

      Time.stub!(:now).and_return(Time.utc(2010, 1, 1, 6, 0, 0))
      topic1 = board.post_topic(user1, :title => "Topic 1", :message => "First post for Topic 1", :sticky => 1)

      Time.stub!(:now).and_return(Time.utc(2010, 1, 1, 7, 0, 0))
      topic2 = board.post_topic(user1, :title => "Topic 2", :message => "First post for Topic 2")
      post   = topic2.posts.first

      Time.stub!(:now).and_return(Time.utc(2010, 1, 1, 8, 0, 0))
      post.reply(user2, :message => "Second post for Topic 2")
    end
