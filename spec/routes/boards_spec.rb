...
describe Iceberg::Routes::Boards do
  describe "new board" do
    ...
    describe "GET" do
      ...
      it "should NOT have a hidden field for the parent if there is no parent" do
        get "/boards/new"
        last_response.body.should_not have_xpath("//form/input[@name='iceberg-board[parent_id]'][@type='hidden']")
      end
    ...
    describe "POST" do
      it "should be successful" do
        post "/boards", {'iceberg-board' => {'title' => 'New Board', 'description' => 'My new board', 'allow_topics' => 0}}
        follow_redirect!
        last_response.should be_ok
      end

      it "should be successful with a parent" do
        post "/boards", {'iceberg-board' => {'parent_id' => @root.id, 'title' => 'New Board', 'description' => 'My new board', 'allow_topics' => 1}}
        follow_redirect!
        last_response.should be_ok
      end
      ...
      it "should render the new board page if creation fails" do
        post "/boards", {'iceberg-board' => {'title' => 'Root', 'description' => 'Going to fail', 'allow_topics' => 0}}
        last_request.path.should  == "/boards"
  ...
  describe "edit board" do
    ...
    describe "GET" do
      ...
      it "should have a text field for the title" do
        get "/boards/#{@board.id}/edit"
        last_response.body.should have_xpath("//form/input[@name='iceberg-board[title]'][@type='text'][@value='Board']")
      end
      
      it "should have a text area for description" do
        get "/boards/#{@board.id}/edit"
        last_response.body.should have_xpath("//form/textarea[@name='iceberg-board[description]'][contains(text(), 'First board')]")
      end
      
      it "should have a checkbox for allowing topics" do
        get "/boards/#{@board.id}/edit"
        last_response.body.should have_xpath("//form/input[@name='iceberg-board[allow_topics]'][@type='checkbox'][@value='1']")
        last_response.body.should have_xpath("//form/input[@name='iceberg-board[allow_topics]'][@type='hidden'][@value='0']")
      end
    end
