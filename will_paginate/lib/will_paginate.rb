require File.expand_path(File.join(File.dirname(__FILE__), 'will_paginate', 'deprecation'))
...
module WillPaginate
  ...
  def self.enable_named_scope(patch = true)
    ...
    require File.expand_path(File.join(File.dirname(__FILE__), 'will_paginate', 'finders', 'active_record', 'named_scope'))
    require File.expand_path(File.join(File.dirname(__FILE__), 'will_paginate', 'finders', 'active_record', 'named_scope_patch')) if patch
...
if defined?(Rails)
  require File.expand_path(File.join(File.dirname(__FILE__), 'will_paginate', 'view_helpers', 'action_view')) if defined?(ActionController)
  require File.expand_path(File.join(File.dirname(__FILE__), 'will_paginate', 'finders', 'active_record')) if defined?(ActiveRecord)
end

if defined?(Merb::Plugins)
  require File.expand_path(File.join(File.dirname(__FILE__), 'will_paginate', 'view_helpers', 'merb'))
  # auto-load the right ORM adapter
  if adapter = { :datamapper => 'data_mapper', :activerecord => 'active_record', :sequel => 'sequel' }[Merb.orm]
    require File.expand_path(File.join(File.dirname(__FILE__), 'will_paginate', 'finders', adapter))
  end
end
